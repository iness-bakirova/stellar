...
import { LuFileSpreadsheet, LuChevronUp, LuChevronDown, LuX, LuWifiOff } from "react-icons/lu";
...
const ManageTasks = () => {
  ...
  const getAllUsers = async () => {
    try {
      ...
      if (!navigator.onLine) {
        ...
        setError("Не удалось загрузить список пользователей. Пожалуйста, обновите страницу.");
      }
    }
  };
  ...
  useEffect(() => {
    getAllTasks();
  }, [filterStatus, filterPriority, filterUser, searchQuery]);

  return (
    <DashboardLayout activeMenu="Управлять задачами">
      <div className="my-3">
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-2">
              ...
              <div className="relative">
                ...
                >
                  {users.map(user => (
                    <option key={user.value} value={user.value} className="flex items-center gap-2">
                      {user.label}
                    </option>
                  ))}
                </select>
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col -space-y-1">
                  {users.findIndex(user => user.value === filterUser) > 0 && (
                    <LuChevronUp 
                      className="text-lg cursor-pointer hover:text-primary" 
                      onClick={() => {
                        const currentIndex = users.findIndex(user => user.value === filterUser);
                        setFilterUser(users[currentIndex - 1].value);
                      }} 
                    />
                  )}
                  {users.findIndex(user => user.value === filterUser) < users.length - 1 && (
                    <LuChevronDown 
                      className="text-lg cursor-pointer hover:text-primary" 
                      onClick={() => {
                        const currentIndex = users.findIndex(user => user.value === filterUser);
            ...
            <button 
              className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-md text-sm hover:bg-primary/90 h-[38px]"
        ...
        {error ? (
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            {(filterStatus !== "Все" || filterPriority !== "All" || filterUser !== "All" || searchQuery) && (
              <button 
